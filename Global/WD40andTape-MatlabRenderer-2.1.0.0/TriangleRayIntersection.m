function [intersect, t, N] = TriangleRayIntersection(orig, dir, vert0, vert1, vert2)
...
for i=1:nv
    ...
    if ~isempty(v1)
        intersect(i)=true;
        [t(i),idx] = min(sum(edge2(v1,:).*qvec(v1,:),2)./det(v1,:)); %"first hit" range scalar along direction "dir"
        N(i) = v1(idx);
    end
end
